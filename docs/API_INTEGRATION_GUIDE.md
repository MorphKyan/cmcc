...
| `404 Not Found` | 资源未找到 |
| `409 Conflict` | 操作冲突 (如重复初始化) |
| `500 Internal Server Error` | 服务器内部错误 |
| `503 Service Unavailable` | 服务不可用 (如 RAG 未初始化) |
...
1. **初始化顺序**: 建议按照 区域 → 设备 → 门禁 → 媒体 的顺序上传数据，因为设备依赖区域信息。
2. **RAG 刷新**: 批量上传数据后系统会自动刷新 RAG 数据库，无需手动调用刷新接口。
3. **位置更新**: 确保客户端已建立 WebSocket 连接后再调用位置更新接口。
4. **状态检查**: 在调用 RAG 查询接口前，建议先检查 `/rag/status` 确认服务状态为 `READY`。
