...
这是一个基于实时语音识别和大型语言模型（LLM）的智能控制系统，专为中国移动智慧展厅设计。它能够将用户的自然语言语音指令，通过RAG（检索增强生成）技术和本地知识库，精确地转换为结构化的JSON指令或函数调用列表，用于控制展厅内的设备。
...
1.  **配置 (`src/config/config.py`)**: 集中管理所有配置项，如API密钥、模型路径、Prompt模板、音频参数等。包含两个版本的系统提示模板，V2版本支持复合指令处理。
...
1.  **API密钥**: 如果使用火山引擎模型，需要在 `src/config/config.py` 中配置 `ARK_API_KEY`。
...
- `POST /api/rag/reinitialize` - 异步触发RAG处理器的重新初始化（后台任务）
...
-   **实时处理**: 采用异步事件循环（WebSocket）处理音频流、VAD、ASR、RAG和LLM调用，保证实时性。
-   **错误处理**: 各模块包含基本的错误处理逻辑，确保程序稳定性。
-   **Prompt Engineering**: 使用详细的系统提示模板指导LLM行为，支持Function Calling，能够处理复合指令。V2模板支持生成函数调用列表以处理多意图指令。
-   **API设计**: 遵循RESTful API设计原则，提供清晰的接口文档和错误处理机制。
-   **前端集成**: 项目包含一个Vue.js前端应用 (`frontend/vue-project`)，用于通过WebSocket与后端API服务进行交互。
-   **状态管理**: RAG处理器具有明确的状态管理（UNINITIALIZED, INITIALIZING, READY, ERROR），确保服务可靠性。