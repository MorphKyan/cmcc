...
def download_file(url, filepath):
    ...
    # Only apply HF mirror substitution for HuggingFace URLs
    if "huggingface.co" in url:
        ...
        try:
            ...
            print(f"Failed via mirror {mirror_url}, trying direct: {e}")

    # Direct download (used for GitHub or as HF fallback)
    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
    try:
        with urllib.request.urlopen(req, context=context, timeout=120) as response, open(filepath, 'wb') as out_file:
            import shutil
            shutil.copyfileobj(response, out_file)
...
def main():
    ...
    # Silero VAD Model
    # The official source is the sherpa-onnx GitHub releases
