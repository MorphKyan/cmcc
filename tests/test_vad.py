...
class VADTestSuite:
    ...
    def test_irregular_chunk_sizes(self):
        ...
        # 持续处理直到整个speech被完整处理
        while total_processed < len(speech):
            # 随机选择块大小（在100到1200采样点之间）
            chunk_size = random.randint(500, 1200)
        ...
        # 检查是否检测到了语音段
        if all_segments:
            print(f"不规则大小音频块测试通过: 总共检测到 {all_segments} 个语音段")
            return True
        else:
    ...
    def run_all_tests(self):
        ...
        all_tests_passed = (model_test_passed and chunk_test_passed and 
                           irregular_chunk_test_passed and boundary_test_passed and file_test_passed)
