<template>
  <div class="manager-section">
    <div class="manager-header">
      ...
      <p class="manager-desc">管理和批量上传区域信息</p>
    </div>
    ...
    <div class="list-card card">
      <div class="card-header">
        ...
        <div class="header-actions">
          <button class="btn btn-secondary btn-sm" @click="fetchAreas">
            🔄 刷新列表
          </button>
          <button class="btn btn-danger btn-sm" @click="clearAllAreas">
            🗑️ 清空所有
          </button>
        </div>
      </div>
      ...
      <div v-else-if="areas.length === 0" class="no-data">
      ...
      <div class="list-footer">
        <span class="count-badge">共 {{ areas.length }} 条记录</span>
      </div>
    </div>
...
<script>
import { uploadAreasBatch, getAreas, clearAreas } from '../api'
  ...
  methods: {
    async fetchAreas() {
      ...
      try {
        ...
        this.message = '获取区域列表失败: ' + error.message
        this.status = 'error'
      } finally {
    ...
    async upload() {
      ...
      try {
        ...
        const response = await uploadAreasBatch(items)
    ...
    async clearAllAreas() {
      if (!confirm('确定要清空所有区域数据吗？此操作不可恢复！')) {
        return
      }
      try {
        await clearAreas()
