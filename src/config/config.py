...
*   **工具选择原则**：对于"打开"、"关闭"等操作，**只有当**用户的指令内容与设备的`command`列表中的某一项有**较强的语义对应关系**时（例如用户说"全部打开"对应"全部开启"），才使用`device_custom_command`。如果用户只是泛泛地说"打开"或"关闭"，且没有匹配到更具体的自定义命令，请优先使用`control_power`。
"""
