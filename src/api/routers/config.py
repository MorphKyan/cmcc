from fastapi import APIRouter, HTTPException, status
from loguru import logger

from src.api.schemas import ConfigResponse
from src.config.config import get_settings

router = APIRouter(
    prefix="/config",
    tags=["Configuration"]
)


@router.get("/current", response_model=ConfigResponse)
async def get_current_config() -> ConfigResponse:
    """
    获取当前后端服务的配置。
    """
    try:
        settings = get_settings()

        # 组织配置数据按类别分组
        config_data = {
            "vad": {},
            "asr": {},
            "rag": {},
            "llm": {}
        }

        # VAD 配置
        config_data["vad"] = {
            "chunk_size": settings.vad.chunk_size,
            "sample_rate": settings.vad.sample_rate,
            "model": settings.vad.model,
            "max_single_segment_time": settings.vad.max_single_segment_time,
            "save_audio_segments": settings.vad.save_audio_segments,
        }

        # ASR 配置
        config_data["asr"] = {
            "model_dir": settings.asr.model_dir,
            "sample_rate": settings.asr.sample_rate,
            "language": settings.asr.language,
            "use_itn": settings.asr.use_itn,
            "spk_model_path": settings.asr.spk_model_path,
            "spk_sim_threshold": settings.asr.spk_sim_threshold,
            "fallback_speaker_label": settings.asr.fallback_speaker_label,
            "batch_size": settings.asr.batch_size,
            "hotwords": settings.asr.hotwords
        }

        # RAG 配置
        config_data["rag"] = {
            "provider": settings.rag.provider,
            "videos_data_path": settings.data.media_data_path,
            "chroma_db_dir": settings.rag.chroma_db_dir,
            "top_k_results": settings.rag.top_k_results,
            "ollama_base_url": settings.rag.ollama_base_url,
            "ollama_embedding_model": settings.rag.ollama_embedding_model,
            "openai_compatible_embedding_model": settings.rag.openai_compatible_embedding_model,
            "openai_compatible_base_url": settings.rag.openai_compatible_base_url,
            "openai_compatible_api_key": "[REDACTED]" if settings.rag.openai_compatible_api_key else None,
            "openai_compatible_chunk_size": settings.rag.openai_compatible_chunk_size,
            "modelscope_embedding_model": settings.rag.modelscope_embedding_model,
            "modelscope_base_url": settings.rag.modelscope_base_url,
            "modelscope_api_key": "[REDACTED]" if settings.rag.modelscope_api_key else None,
            "dashscope_embedding_model": settings.rag.dashscope_embedding_model,
            "dashscope_base_url": settings.rag.dashscope_base_url,
            "dashscope_api_key": "[REDACTED]" if settings.rag.dashscope_api_key else None,
        }

        # LLM 配置
        config_data["llm"] = {
            "provider": settings.llm.provider,
            "openai_compatible_model": settings.llm.openai_compatible_model,
            "openai_compatible_base_url": settings.llm.openai_compatible_base_url,
            "openai_compatible_api_key": "[REDACTED]" if settings.llm.openai_compatible_api_key else None,
            "ollama_model": settings.llm.ollama_model,
            "ollama_base_url": settings.llm.ollama_base_url,
            "modelscope_model": settings.llm.modelscope_model,
            "modelscope_base_url": settings.llm.modelscope_base_url,
            "modelscope_api_key": "[REDACTED]" if settings.llm.modelscope_api_key else None,
            "dashscope_model": settings.llm.dashscope_model,
            "dashscope_base_url": settings.llm.dashscope_base_url,
            "dashscope_api_key": "[REDACTED]" if settings.llm.dashscope_api_key else None,
            "max_validation_retries": settings.llm.max_validation_retries,
            "retry_delay": settings.llm.retry_delay,
            "request_timeout": settings.llm.request_timeout,
            "connection_timeout": settings.llm.connection_timeout,
        }

        return ConfigResponse(
            status="success",
            data=config_data,
            message="成功获取当前配置"
        )

    except Exception as e:
        logger.exception("获取配置时发生错误")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"获取配置时发生错误: {str(e)}"
        )
