...
app = FastAPI(title="RAG API Service", description="基于FastAPI的RAG服务")
...
@app.post("/api/rag/refresh", summary="刷新RAG数据库")
...
@app.get("/api/rag/status", summary="获取RAG状态")
...
@app.post("/api/rag/query", summary="查询RAG数据库")
async def query_rag(query: dict):
    ...
    try:
        ...
        if not query_text:
            raise HTTPException(status_code=400, detail="缺少查询参数 'query'")
...
@app.post("/api/rag/upload-videos", summary="上传videos.csv文件并更新RAG数据库")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='RAG API Service (FastAPI)')
